...
def main() -> None:
    ...
    i=1
    for seq in gen_seq(weights, args.number, args.kmer, args.min_len, args.max_len):
        out_file.write(f'>{i}\n')
...
# --------------------------------------------------
def gen_seq(weighting: Weights, number: int, k: int, min_len: int, max_len: int) -> Optional[str]:
    """ Generate a sequence """
    ...
    for _ in range(number):
        ...
        while len(seq) < seq_len:
            if seq[-k+1:] not in list(weighting.keys()):
                break
